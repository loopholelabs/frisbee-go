...
func writeClient(f File, services protoreflect.ServiceDescriptors) {
	...
	f.P("func NewClient(tlsConfig *tls.Config, logger *zerolog.Logger) (*Client, error) {")
	...
	f.P(tab, tab, "c.Client, err = frisbee.NewClient(table, context.Background(), frisbee.WithTLS(tlsConfig), frisbee.WithLogger(logger))")
	f.P(tab, tab, "if err != nil {")
	f.P(tab, tab, tab, "return nil, err")
	f.P(tab, tab, "}")
	f.P(tab, "} else {")
	f.P(tab, tab, "c.Client, err = frisbee.NewClient(table, context.Background(), frisbee.WithLogger(logger))")
	f.P(tab, tab, "if err != nil {")
	f.P(tab, tab, tab, "return nil, err")
	f.P(tab, tab, "}")
	f.P(tab, "}")
