...
func TestMessageV0EncodeDecode(t *testing.T) {
	...
	binary.BigEndian.PutUint16(correct[1:3], MagicBytes)
	correct[3] = Version0
	binary.BigEndian.PutUint16(correct[4:6], uint16(16))
...
func TestEncodeDecodeHandlerV0(t *testing.T) {
	...
	assert.Equal(t, Version0, message.Version)
	...
	assert.Equal(t, Version0, emptyMessage.Version)
	assert.Equal(t, uint16(16), message.Id)
...
func BenchmarkEncodeHandler(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = handlerV0.Encode(uint16(i), MessagePacket, 0, 512)
	}
}
...
func BenchmarkEncodeDecodeHandler(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		encodedMessage, _ := handlerV0.Encode(uint16(i), MessagePacket, 0, 512)
		_, _ = handlerV0.Decode(encodedMessage[:])
	}
}
...
func BenchmarkEncode(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = EncodeV0(uint16(i), MessagePacket, 0, 512)
	}
}
...
func BenchmarkEncodeDecode(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		encodedMessage, _ := EncodeV0(uint16(i), MessagePacket, 0, 512)
		_, _ = DecodeV0(encodedMessage[:])
	}
}
